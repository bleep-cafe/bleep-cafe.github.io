 ...
 * LICENSE file in the root directory of this source tree.
 */
var f = react.exports, k$1 = Symbol.for("react.element"), l$1 = Symbol.for("react.fragment"), m$2 = Object.prototype.hasOwnProperty, n$2 = f.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, p$3 = { key: true, ref: true, __self: true, __source: true };
function q$2(c, a, g) {
...
"useSyncExternalStore" in $2AODx$react ? ((module) => module.useSyncExternalStore)($2AODx$react) : shim$1;
...
const Link = /* @__PURE__ */ react.exports.forwardRef(function LinkWithRef(_ref4, ref2) {
  let {
    onClick,
    relative,
    reloadDocument,
...
function node(type2, properties = [], connections = []) {
  return { type: type2, properties, connections };
}
function keyed(key, node2) {
  return key ? { key, ...node2 } : node2;
}
function ref(key) {
...
class VirtualAudioContext {
  constructor(ctx2 = new AudioContext()) {
    this.ctx = ctx2;
    this.prev = {};
    this.nodes = ctx2.nodes = {};
  }
  update(graph = []) {
    ...
    patch.created.forEach((created) => {
      switch (created.type) {
        ...
        case "AudioParam": {
          ...
          this.nodes[created.key][created.label].linearRampToValueAtTime(
            created.value,
  ...
  createNode(key, type2, properties, connections) {
    const node2 = (() => {
      var _a, _b;
      switch (type2) {
        ...
        case "DelayNode": {
          const maxDelayTime = (_b = (_a = properties.find((p2) => p2.label === "maxDelayTime")) == null ? void 0 : _a.value) != null ? _b : 1;
        ...
        default: {
          if (type2 in window && window[type2].prototype instanceof window.AudioNode) {
            console.warn(
              `AudioNodes of type ${type2} are not currently supported.`,
              `Please consider opening a PR if you"re interested in adding support for this node.`,
              `Creating a dummy gain node instead.`
            );
          } else {
            console.warn(
              `${type2} is not a recognised AudioNode.`,
    ...
    for (const { type: type3, label, value } of properties) {
      switch (type3) {
        case "NodeProperty":
          node2[label] = value;
          break;
        ...
        case "ScheduledUpdate": {
          if (label in node2) {
            node2[label][value.method](value.target, value.time);
          } else {
            window.setTimeout(
              () => node2[label] = value.target,
    ...
    if ("start" in node2) {
      node2.start(this.ctx.currentTime);
    }
    return this.nodes[key] = this.ctx.nodes[key] = node2;
  }
  connect(from = "", to = "") {
...
function flatten(base = "") {
  return (nodes, node2, idx) => {
    if (node2.type === "RefNode") {
      return nodes;
    }
    const key = node2.key || base + idx;
    const connections = node2.connections.map(
      (connection, i) => connection.key || key + i
    );
    return node2.connections.reduce(flatten(key), {
      ...nodes,
      [key]: { ...node2, key, connections }
    });
  };
}
  ...
  Object.keys(curr).forEach((key) => {
    if (key in prev) {
      ...
      if (prevNode.type !== currNode.type) {
        ...
        nodePatch.created.forEach(
          (created) => patch.created.push({ key, ...created })
        );
        nodePatch.deleted.forEach(
          (deleted) => patch.deleted.push({ key, ...deleted })
        );
      }
    } else {
...
function diffNode(prev, curr) {
  ...
  const prevNodeProperties = prev.properties.filter(({ type: type2 }) => type2 === "NodeProperty").reduce((props, prop) => ({ ...props, [prop.label]: prop }), {});
  const currNodeProperties = curr.properties.filter(({ type: type2 }) => type2 === "NodeProperty").reduce((props, prop) => ({ ...props, [prop.label]: prop }), {});
  ...
  const prevAudioParams = prev.properties.filter(({ type: type2 }) => type2 === "AudioParam").reduce((params, param2) => ({ ...params, [param2.label]: param2 }), {});
  const currAudioParams = curr.properties.filter(({ type: type2 }) => type2 === "AudioParam").reduce((params, param2) => ({ ...params, [param2.label]: param2 }), {});
  ...
  curr.properties.filter(({ type: type2 }) => type2 === "ScheduledUpdate").forEach((update) => {
    const existsOnPrev = prev.properties.some(
      ({ type: type2, label, value }) => type2 === update.type && label === update.label && value.method == update.value.method && value.time === update.value.time && value.target === update.value.target
    );
    if (!existsOnPrev) {
...
function useStore$1(api, selector2 = api.getState, equalityFn) {
...
function selection_merge(context) {
  ...
  for (var groups0 = this._groups, groups1 = selection2._groups, m0 = groups0.length, m1 = groups1.length, m2 = Math.min(m0, m1), merges = new Array(m0), j = 0; j < m2; ++j) {
    for (var group0 = groups0[j], group1 = groups1[j], n2 = group0.length, merge = merges[j] = new Array(n2), node2, i = 0; i < n2; ++i) {
...
function hslConvert(o) {
  ...
  if (o instanceof Hsl)
    return o;
  o = o.rgb();
  var r2 = o.r / 255, g = o.g / 255, b = o.b / 255, min2 = Math.min(r2, g, b), max2 = Math.max(r2, g, b), h2 = NaN, s = max2 - min2, l2 = (max2 + min2) / 2;
  if (s) {
    if (r2 === max2)
      h2 = (g - b) / s + (g < b) * 6;
    else if (g === max2)
...
function styleTween(name2, value, priority) {
  ...
  function tween() {
    var i = value.apply(this, arguments);
    if (i !== i0)
      t2 = (i0 = i) && styleInterpolate(name2, i, priority);
    return t2;
  }
  tween._value = value;
  return tween;
...
const $7SXl2$reactdom = /* @__PURE__ */ getDefaultExportFromCjs(reactDom.exports);
...
function useStore(selector2, equalityFn) {
  const store = react.exports.useContext(StoreContext);
  if (store === null) {
    throw new Error(errorMessage);
  }
  return useStore$1(store, selector2, equalityFn);
}
const useStoreApi = () => {
  const store = react.exports.useContext(StoreContext);
  if (store === null) {
...
function Panel({ position, children: children2, className, style: style2, ...rest }) {
  ...
  return jsx("div", { className: cc$1(["react-flow__panel", className, ...positionClasses]), style: { ...style2, pointerEvents }, ...rest, children: children2 });
}
...
const EdgeText = ({ x: x2, y: y2, label, labelStyle = {}, labelShowBg = true, labelBgStyle = {}, labelBgPadding = [2, 4], labelBgBorderRadius = 2, children: children2, className, ...rest }) => {
  const edgeRef = react.exports.useRef(null);
  const [edgeTextBbox, setEdgeTextBbox] = react.exports.useState({ x: 0, y: 0, width: 0, height: 0 });
  const edgeTextClasses = cc$1(["react-flow__edge-textwrapper", className]);
  react.exports.useEffect(() => {
    if (edgeRef.current) {
  ...
  return jsxs("g", { transform: `translate(${x2 - edgeTextBbox.width / 2} ${y2 - edgeTextBbox.height / 2})`, className: edgeTextClasses, visibility: edgeTextBbox.width ? "visible" : "hidden", ...rest, children: [labelShowBg && jsx("rect", { width: edgeTextBbox.width + 2 * labelBgPadding[0], x: -labelBgPadding[0], y: -labelBgPadding[1], height: edgeTextBbox.height + 2 * labelBgPadding[1], className: "react-flow__edge-textbg", style: labelBgStyle, rx: labelBgBorderRadius, ry: labelBgBorderRadius }), jsx("text", { className: "react-flow__edge-text", y: edgeTextBbox.height / 2, dy: "0.3em", ref: edgeRef, style: labelStyle, children: label }), children2] });
};
var EdgeText$1 = react.exports.memo(EdgeText);
const BaseEdge = ({ path, labelX, labelY, label, labelStyle, labelShowBg, labelBgStyle, labelBgPadding, labelBgBorderRadius, style: style2, markerEnd, markerStart, interactionWidth = 20 }) => {
  return jsxs(Fragment, { children: [jsx("path", { style: style2, d: path, fill: "none", className: "react-flow__edge-path", markerEnd, markerStart }), interactionWidth && jsx("path", { d: path, fill: "none", strokeOpacity: 0, strokeWidth: interactionWidth, className: "react-flow__edge-interaction" }), label ? jsx(EdgeText$1, { x: labelX, y: labelY, label, labelStyle, labelShowBg, labelBgStyle, labelBgPadding, labelBgBorderRadius }) : null] });
};
...
const clamp = (val, min2 = 0, max2 = 1) => Math.min(Math.max(val, min2), max2);
...
const SimpleBezierEdge = react.exports.memo(({ sourceX, sourceY, targetX, targetY, sourcePosition = Position.Bottom, targetPosition = Position.Top, label, labelStyle, labelShowBg, labelBgStyle, labelBgPadding, labelBgBorderRadius, style: style2, markerEnd, markerStart, interactionWidth }) => {
  ...
  return jsx(BaseEdge, { path, labelX, labelY, label, labelStyle, labelShowBg, labelBgStyle, labelBgPadding, labelBgBorderRadius, style: style2, markerEnd, markerStart, interactionWidth });
});
...
const SmoothStepEdge = react.exports.memo(({ sourceX, sourceY, targetX, targetY, label, labelStyle, labelShowBg, labelBgStyle, labelBgPadding, labelBgBorderRadius, style: style2, sourcePosition = Position.Bottom, targetPosition = Position.Top, markerEnd, markerStart, pathOptions, interactionWidth }) => {
...
const StepEdge = react.exports.memo((props) => {
  var _a;
  return jsx(SmoothStepEdge, { ...props, pathOptions: react.exports.useMemo(() => {
    var _a2;
...
const StraightEdge = react.exports.memo(({ sourceX, sourceY, targetX, targetY, label, labelStyle, labelShowBg, labelBgStyle, labelBgPadding, labelBgBorderRadius, style: style2, markerEnd, markerStart, interactionWidth }) => {
...
const BezierEdge = react.exports.memo(({ sourceX, sourceY, targetX, targetY, sourcePosition = Position.Bottom, targetPosition = Position.Top, label, labelStyle, labelShowBg, labelBgStyle, labelBgPadding, labelBgBorderRadius, style: style2, markerEnd, markerStart, pathOptions, interactionWidth }) => {
...
const NodeIdContext = react.exports.createContext(null);
...
const getNodesInside = (nodeInternals, rect, [tx, ty, tScale] = [0, 0, 1], partially = false, excludeNonSelectableNodes = false) => {
  const paneRect = {
    x: (rect.x - tx) / tScale,
    y: (rect.y - ty) / tScale,
  ...
  nodeInternals.forEach((node2) => {
    const { positionAbsolute = { x: 0, y: 0 }, width, height, selectable = true } = node2;
...
const Handle = react.exports.forwardRef(({ type: type2 = "source", position = Position.Top, isValidConnection = alwaysValid, isConnectable = true, id: id2, onConnect, children: children2, className, onMouseDown, ...rest }, ref2) => {
  const store = useStoreApi();
  const nodeId = react.exports.useContext(NodeIdContext);
  ...
  const onClick = (event) => {
    ...
    if (!connectionStartHandle) {
      ...
      store.setState({ connectionStartHandle: { nodeId, type: type2, handleId } });
      return;
    }
    const doc2 = getHostForElement(event.target);
  ...
  return jsx("div", { "data-handleid": handleId, "data-nodeid": nodeId, "data-handlepos": position, className: cc$1([
    "react-flow__handle",
    ...
    className,
    {
      ...
      connecting: (connectionStartHandle == null ? void 0 : connectionStartHandle.nodeId) === nodeId && (connectionStartHandle == null ? void 0 : connectionStartHandle.handleId) === handleId && (connectionStartHandle == null ? void 0 : connectionStartHandle.type) === type2
    }
  ]), onMouseDown: onMouseDownHandler, onClick: connectOnClick ? onClick : void 0, ref: ref2, ...rest, children: children2 });
});
Handle.displayName = "Handle";
var Handle$1 = react.exports.memo(Handle);
const DefaultNode = ({ data, isConnectable, targetPosition = Position.Top, sourcePosition = Position.Bottom }) => {
  return jsxs(Fragment, { children: [jsx(Handle$1, { type: "target", position: targetPosition, isConnectable }), data == null ? void 0 : data.label, jsx(Handle$1, { type: "source", position: sourcePosition, isConnectable })] });
};
DefaultNode.displayName = "DefaultNode";
var DefaultNode$1 = react.exports.memo(DefaultNode);
const InputNode = ({ data, isConnectable, sourcePosition = Position.Bottom }) => jsxs(Fragment, { children: [data == null ? void 0 : data.label, jsx(Handle$1, { type: "source", position: sourcePosition, isConnectable })] });
InputNode.displayName = "InputNode";
var InputNode$1 = react.exports.memo(InputNode);
const OutputNode = ({ data, isConnectable, targetPosition = Position.Top }) => jsxs(Fragment, { children: [jsx(Handle$1, { type: "target", position: targetPosition, isConnectable }), data == null ? void 0 : data.label] });
OutputNode.displayName = "OutputNode";
var OutputNode$1 = react.exports.memo(OutputNode);
const GroupNode = () => null;
GroupNode.displayName = "GroupNode";
const selector$c = (s) => ({
  selectedNodes: Array.from(s.nodeInternals.values()).filter((n2) => n2.selected),
...
const SelectionListener = react.exports.memo(({ onSelectionChange }) => {
  const store = useStoreApi();
...
function Wrapper$1({ onSelectionChange }) {
  ...
  if (onSelectionChange || storeHasSelectionChange) {
    return jsx(SelectionListener, { onSelectionChange });
  }
  return null;
}
const selector$b = (s) => ({
...
const EdgeAnchor = ({ position, centerX, centerY, radius = 10, onMouseDown, onMouseEnter, onMouseOut, type: type2 }) => jsx("circle", { onMouseDown, onMouseEnter, onMouseOut, className: cc$1([EdgeUpdaterClassName, `${EdgeUpdaterClassName}-${type2}`]), cx: shiftX(centerX, radius, position), cy: shiftY(centerY, radius, position), r: radius, stroke: "transparent", fill: "transparent" });
var wrapEdge = (EdgeComponent) => {
  const EdgeWrapper = ({ id: id2, className, type: type2, data, onClick, onEdgeDoubleClick, selected, animated, label, labelStyle, labelShowBg, labelBgStyle, labelBgPadding, labelBgBorderRadius, style: style2, source, target, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, elementsSelectable, hidden, sourceHandleId, targetHandleId, onContextMenu, onMouseEnter, onMouseMove, onMouseLeave, edgeUpdaterRadius, onEdgeUpdate, onEdgeUpdateStart, onEdgeUpdateEnd, markerEnd, markerStart, rfId, ariaLabel, isFocusable, pathOptions, interactionWidth }) => {
    const edgeRef = react.exports.useRef(null);
    const [updateHover, setUpdateHover] = react.exports.useState(false);
    const [updating, setUpdating] = react.exports.useState(false);
    const store = useStoreApi();
    const markerStartUrl = react.exports.useMemo(() => `url(#${getMarkerId(markerStart, rfId)})`, [markerStart, rfId]);
    const markerEndUrl = react.exports.useMemo(() => `url(#${getMarkerId(markerEnd, rfId)})`, [markerEnd, rfId]);
    if (hidden) {
      return null;
    }
    ...
    const handleEdgeUpdater = (event, isSourceHandle) => {
      ...
      const edge = store.getState().edges.find((e2) => e2.id === id2);
      setUpdating(true);
...
function getNodeData(node2) {
  var _a, _b, _c, _d, _e;
  const handleBounds = ((_a = node2 == null ? void 0 : node2[internalsSymbol]) == null ? void 0 : _a.handleBounds) || null;
  const isInvalid = !node2 || !handleBounds || !node2.width || !node2.height || typeof ((_b = node2.positionAbsolute) == null ? void 0 : _b.x) === "undefined" || typeof ((_c = node2.positionAbsolute) == null ? void 0 : _c.y) === "undefined";
  return [
    {
      x: ((_d = node2 == null ? void 0 : node2.positionAbsolute) == null ? void 0 : _d.x) || 0,
      y: ((_e = node2 == null ? void 0 : node2.positionAbsolute) == null ? void 0 : _e.y) || 0,
      width: (node2 == null ? void 0 : node2.width) || 0,
      height: (node2 == null ? void 0 : node2.height) || 0
    },
    handleBounds,
    !isInvalid
  ];
...
function getDragItems(nodeInternals, mousePos, nodeId) {
  return Array.from(nodeInternals.values()).filter((n2) => (n2.selected || n2.id === nodeId) && (!n2.parentNode || !isParentSelected(n2, nodeInternals))).map((n2) => {
    var _a, _b, _c, _d;
    return {
      id: n2.id,
      ...
      distance: {
        x: mousePos.x - ((_b = (_a = n2.positionAbsolute) == null ? void 0 : _a.x) != null ? _b : 0),
        y: mousePos.y - ((_d = (_c = n2.positionAbsolute) == null ? void 0 : _c.y) != null ? _d : 0)
      },
      delta: {
        x: 0,
        y: 0
...
function calcNextPosition(node2, nextPosition, nodeInternals, nodeExtent) {
  ...
  if (node2.extent === "parent") {
    if (node2.parentNode && node2.width && node2.height) {
      ...
      currentExtent = (parent == null ? void 0 : parent.positionAbsolute) && (parent == null ? void 0 : parent.width) && (parent == null ? void 0 : parent.height) ? [
        ...
        [
          parent.positionAbsolute.x + parent.width - node2.width,
          parent.positionAbsolute.y + parent.height - node2.height
        ]
      ] : currentExtent;
    } else {
...
function getEventHandlerParams({ nodeId, dragItems, nodeInternals }) {
  const extentedDragItems = dragItems.map((n2) => {
    const node2 = nodeInternals.get(n2.id);
    return {
      ...node2,
...
function getMouseHandler(id2, getState, handler) {
  return handler === void 0 ? handler : (event) => {
    const node2 = getState().nodeInternals.get(id2);
    handler(event, { ...node2 });
  };
}
function handleNodeClick({ id: id2, store, unselect = false }) {
  ...
  const node2 = nodeInternals.get(id2);
  store.setState({ nodesSelectionActive: false });
  if (!node2.selected) {
...
function useDrag({ nodeRef, disabled = false, noDragClassName, handleSelector, nodeId, isSelectable, selectNodesOnDrag }) {
  ...
  const store = useStoreApi();
  const dragItems = react.exports.useRef();
  const lastPos = react.exports.useRef({ x: null, y: null });
  const getPointerPosition = react.exports.useCallback(({ sourceEvent: sourceEvent2 }) => {
        ...
        const dragHandler = drag().on("start", (event) => {
          ...
          if ((lastPos.current.x !== pointerPos.xSnapped || lastPos.current.y !== pointerPos.ySnapped) && dragItems.current) {
            ...
            dragItems.current = dragItems.current.map((n2) => {
              const nextPosition = { x: pointerPos.x - n2.distance.x, y: pointerPos.y - n2.distance.y };
              if (snapToGrid) {
                nextPosition.x = snapGrid[0] * Math.round(nextPosition.x / snapGrid[0]);
                nextPosition.y = snapGrid[1] * Math.round(nextPosition.y / snapGrid[1]);
              }
              const updatedPos = calcNextPosition(n2, nextPosition, nodeInternals, nodeExtent);
...
function useUpdateNodePositions() {
  const store = useStoreApi();
  const updatePositions = react.exports.useCallback((positionDiff) => {
    const { nodeInternals, nodeExtent, updateNodePositions, snapToGrid, snapGrid } = store.getState();
    const selectedNodes = Array.from(nodeInternals.values()).filter((n2) => n2.selected);
    const nodeUpdates = selectedNodes.map((n2) => {
      if (n2.positionAbsolute) {
        const nextPosition = { x: n2.positionAbsolute.x + positionDiff.x, y: n2.positionAbsolute.y + positionDiff.y };
        if (snapToGrid) {
          nextPosition.x = snapGrid[0] * Math.round(nextPosition.x / snapGrid[0]);
          nextPosition.y = snapGrid[1] * Math.round(nextPosition.y / snapGrid[1]);
        }
...
var wrapNode = (NodeComponent) => {
  const NodeWrapper = ({ id: id2, type: type2, data, xPos, yPos, xPosOrigin, yPosOrigin, selected, onClick, onMouseEnter, onMouseMove, onMouseLeave, onContextMenu, onDoubleClick, style: style2, className, isDraggable, isSelectable, isConnectable, isFocusable, selectNodesOnDrag, sourcePosition, targetPosition, hidden, resizeObserver, dragHandle, zIndex, isParent, noDragClassName, noPanClassName, initialized, disableKeyboardA11y, ariaLabel, rfId }) => {
    ...
    const prevSourcePosition = react.exports.useRef(sourcePosition);
    const prevTargetPosition = react.exports.useRef(targetPosition);
    const prevType = react.exports.useRef(type2);
    ...
    const onSelectNodeHandler = (event) => {
      ...
      if (onClick) {
        const node2 = store.getState().nodeInternals.get(id2);
        onClick(event, { ...node2 });
      }
    };
    const onKeyDown = (event) => {
...
var useKeyPress = (keyCode = null, options = { target: doc }) => {
  ...
  const pressedKeys = react.exports.useRef(/* @__PURE__ */ new Set([]));
  const [keyCodes, keysToWatch] = react.exports.useMemo(() => {
    if (keyCode !== null) {
...
function createNodeInternals(nodes, nodeInternals) {
  ...
  nodes.forEach((node2) => {
    var _a;
    const z2 = isNumeric(node2.zIndex) ? node2.zIndex : node2.selected ? 1e3 : 0;
  ...
  nextNodeInternals.forEach((node2) => {
    var _a, _b;
    ...
    if (node2.parentNode || parentNodes[node2.id]) {
      const { x: x2, y: y2, z: z2 } = calculateXYZPosition(node2, nextNodeInternals, parentNodes, {
        ...node2.position,
        z: (_b = (_a = node2[internalsSymbol]) == null ? void 0 : _a.z) != null ? _b : 0
      });
...
function fitView(get2, options = {}) {
  var _a, _b, _c;
  const { nodeInternals, width, height, minZoom, maxZoom, d3Zoom, d3Selection, fitViewOnInitDone, fitViewOnInit } = get2();
  if (options.initial && !fitViewOnInitDone && fitViewOnInit || !options.initial) {
    if (d3Zoom && d3Selection) {
      const nodes = Array.from(nodeInternals.values()).filter((n2) => options.includeHiddenNodes ? n2.width && n2.height : !n2.hidden);
      const nodesInitialized = nodes.every((n2) => n2.width && n2.height);
      if (nodes.length > 0 && nodesInitialized) {
        const bounds = getRectOfNodes(nodes);
        const [x2, y2, zoom2] = getTransformForBounds(bounds, width, height, (_a = options.minZoom) != null ? _a : minZoom, (_b = options.maxZoom) != null ? _b : maxZoom, (_c = options.padding) != null ? _c : 0.1);
...
const useViewportHelper = () => {
  ...
  const viewportHelperFunctions = react.exports.useMemo(() => {
    if (d3Selection && d3Zoom) {
      return {
        ...
        setViewport: (transform, options) => {
          ...
          const nextTransform = identity.translate((_a = transform.x) != null ? _a : x2, (_b = transform.y) != null ? _b : y2).scale((_c = transform.zoom) != null ? _c : zoom2);
        ...
        fitBounds: (bounds, options) => {
          ...
          const [x2, y2, zoom2] = getTransformForBounds(bounds, width, height, minZoom, maxZoom, (_a = options == null ? void 0 : options.padding) != null ? _a : 0.1);
...
function useReactFlow() {
  ...
  const store = useStoreApi();
  const getNodes = react.exports.useCallback(() => {
    const { nodeInternals } = store.getState();
    const nodes = Array.from(nodeInternals.values());
    return nodes.map((n2) => ({ ...n2 }));
  }, []);
  const getNode = react.exports.useCallback((id2) => {
    const { nodeInternals } = store.getState();
    return nodeInternals.get(id2);
  }, []);
  const getEdges = react.exports.useCallback(() => {
    const { edges = [] } = store.getState();
    return edges.map((e2) => ({ ...e2 }));
  }, []);
  const getEdge = react.exports.useCallback((id2) => {
    const { edges = [] } = store.getState();
    return edges.find((e2) => e2.id === id2);
  }, []);
  const setNodes = react.exports.useCallback((payload) => {
  ...
  const setEdges = react.exports.useCallback((payload) => {
  ...
  const addNodes = react.exports.useCallback((payload) => {
  ...
  const addEdges = react.exports.useCallback((payload) => {
  ...
  const toObject = react.exports.useCallback(() => {
  ...
  const deleteElements = react.exports.useCallback(({ nodes: nodesDeleted, edges: edgesDeleted }) => {
    ...
    const nodesToRemove = nodes.reduce((res, node2) => {
      const parentHit = !nodeIds.includes(node2.id) && node2.parentNode && res.find((n2) => n2.id === node2.parentNode);
  ...
  const getNodeRect = react.exports.useCallback((nodeOrRect) => {
  ...
  const getIntersectingNodes = react.exports.useCallback((nodeOrRect, partially = true, nodes) => {
    const [nodeRect, node2, isRect] = getNodeRect(nodeOrRect);
    if (!nodeRect) {
      return [];
    }
    return (nodes || Array.from(store.getState().nodeInternals.values())).filter((n2) => {
      if (!isRect && (n2.id === node2.id || !n2.positionAbsolute)) {
        return false;
      }
  ...
  const isNodeIntersecting = react.exports.useCallback((nodeOrRect, area, partially = true) => {
...
var useGlobalKeyHandler = ({ deleteKeyCode, multiSelectionKeyCode }) => {
  ...
  react.exports.useEffect(() => {
    if (deleteKeyPressed) {
      ...
      const nodes = Array.from(nodeInternals.values());
      const selectedNodes = nodes.filter((node2) => node2.selected);
...
const ZoomPane = ({ onMove, onMoveStart, onMoveEnd, zoomOnScroll = true, zoomOnPinch = true, panOnScroll = false, panOnScrollSpeed = 0.5, panOnScrollMode = PanOnScrollMode.Free, zoomOnDoubleClick = true, selectionKeyPressed, elementsSelectable, panOnDrag = true, defaultViewport, translateExtent, minZoom, maxZoom, zoomActivationKeyCode, preventScrolling = true, children: children2, noWheelClassName, noPanClassName }) => {
  const timerId = react.exports.useRef();
  const store = useStoreApi();
  const isZoomingOrPanning = react.exports.useRef(false);
  const zoomPane = react.exports.useRef(null);
  const prevTransform = react.exports.useRef({ x: 0, y: 0, zoom: 0 });
  ...
  react.exports.useEffect(() => {
    if (zoomPane.current) {
      ...
      const updatedTransform = identity.translate(clampedX, clampedY).scale(clampedZoom);
...
const UserSelection = react.exports.memo(({ selectionKeyPressed }) => {
  const store = useStoreApi();
  const prevSelectedNodesCount = react.exports.useRef(0);
  const prevSelectedEdgesCount = react.exports.useRef(0);
  const containerBounds = react.exports.useRef();
  const [userSelectionRect, setUserSelectionRect] = react.exports.useState(initialRect);
  const { userSelectionActive, elementsSelectable } = useStore(selector$7, shallow);
  ...
  return jsx("div", { className: "react-flow__selectionpane react-flow__container", onMouseDown, onMouseMove, onMouseUp, onMouseLeave, children: userSelectionRect.draw && jsx("div", { className: "react-flow__selection react-flow__container", style: {
...
const selector$6 = (s) => ({
  ...
  userSelectionActive: s.userSelectionActive,
  ...getRectOfNodes(Array.from(s.nodeInternals.values()).filter((n2) => n2.selected))
});
const bboxSelector = (s) => {
  const selectedNodes = Array.from(s.nodeInternals.values()).filter((n2) => n2.selected);
...
function NodesSelection({ onSelectionContextMenu, noPanClassName, disableKeyboardA11y }) {
  ...
  const onContextMenu = onSelectionContextMenu ? (event) => {
    const selectedNodes = Array.from(store.getState().nodeInternals.values()).filter((n2) => n2.selected);
  ...
  return jsx("div", { className: cc$1(["react-flow__nodesselection", "react-flow__container", noPanClassName]), style: {
...
var NodesSelection$1 = react.exports.memo(NodesSelection);
const selector$5 = (s) => s.paneDragging;
function Pane({ onClick, onMouseEnter, onMouseMove, onMouseLeave, onContextMenu, onWheel }) {
  const dragging = useStore(selector$5);
  return jsx("div", { className: cc$1(["react-flow__pane", { dragging }]), onClick, onMouseEnter, onMouseMove, onMouseLeave, onContextMenu, onWheel, style: containerStyle });
}
const selector$4 = (s) => s.nodesSelectionActive;
const FlowRenderer = ({ children: children2, onPaneClick, onPaneMouseEnter, onPaneMouseMove, onPaneMouseLeave, onPaneContextMenu, onPaneScroll, deleteKeyCode, onMove, onMoveStart, onMoveEnd, selectionKeyCode, multiSelectionKeyCode, zoomActivationKeyCode, elementsSelectable, zoomOnScroll, zoomOnPinch, panOnScroll, panOnScrollSpeed, panOnScrollMode, zoomOnDoubleClick, panOnDrag, defaultViewport, translateExtent, minZoom, maxZoom, preventScrolling, onSelectionContextMenu, noWheelClassName, noPanClassName, disableKeyboardA11y }) => {
  const store = useStoreApi();
  const nodesSelectionActive = useStore(selector$4);
  ...
  return jsxs(ZoomPane, { onMove, onMoveStart, onMoveEnd, selectionKeyPressed, elementsSelectable, zoomOnScroll, zoomOnPinch, panOnScroll, panOnScrollSpeed, panOnScrollMode, zoomOnDoubleClick, panOnDrag, defaultViewport, translateExtent, minZoom, maxZoom, zoomActivationKeyCode, preventScrolling, noWheelClassName, noPanClassName, children: [children2, jsx(UserSelection, { selectionKeyPressed }), nodesSelectionActive && jsx(NodesSelection$1, { onSelectionContextMenu, noPanClassName, disableKeyboardA11y }), jsx(Pane, { onClick, onMouseEnter: onPaneMouseEnter, onMouseMove: onPaneMouseMove, onMouseLeave: onPaneMouseLeave, onContextMenu, onWheel })] });
};
FlowRenderer.displayName = "FlowRenderer";
var FlowRenderer$1 = react.exports.memo(FlowRenderer);
function useVisibleNodes(onlyRenderVisible) {
  const nodes = useStore(react.exports.useCallback((s) => onlyRenderVisible ? getNodesInside(s.nodeInternals, { x: 0, y: 0, width: s.width, height: s.height }, s.transform, true) : Array.from(s.nodeInternals.values()), [onlyRenderVisible]));
  return nodes;
}
const selector$3 = (s) => ({
...
const NodeRenderer = (props) => {
  const { nodesDraggable, nodesConnectable, nodesFocusable, elementsSelectable, updateNodeDimensions } = useStore(selector$3, shallow);
  const nodes = useVisibleNodes(props.onlyRenderVisibleElements);
  const resizeObserverRef = react.exports.useRef();
  ...
  return jsx("div", { className: "react-flow__nodes", style: containerStyle, children: nodes.map((node2) => {
...
function groupEdgesByZLevel(edges, nodeInternals, elevateEdgesOnSelect = false) {
  ...
  const levelLookup = edges.reduce((tree, edge) => {
    ...
    if (elevateEdgesOnSelect) {
      z2 = hasZIndex ? edge.zIndex : Math.max(((_b = (_a = nodeInternals.get(edge.source)) == null ? void 0 : _a[internalsSymbol]) == null ? void 0 : _b.z) || 0, ((_d = (_c = nodeInternals.get(edge.target)) == null ? void 0 : _c[internalsSymbol]) == null ? void 0 : _d.z) || 0);
    }
    if (tree[z2]) {
      tree[z2].push(edge);
    } else {
...
function useVisibleEdges(onlyRenderVisible, nodeInternals, elevateEdgesOnSelect) {
  const edges = useStore(react.exports.useCallback((s) => {
...
const ConnectionLine = ({ connectionNodeId, connectionHandleType, connectionLineStyle, connectionLineType = ConnectionLineType.Bezier, isConnectable, CustomConnectionLineComponent }) => {
  var _a, _b, _c, _d, _e;
  const { fromNode, handleId, toX, toY } = useStore(react.exports.useCallback((s) => ({
  ...
  if (!fromNode || !isConnectable || !(fromHandleBounds == null ? void 0 : fromHandleBounds[connectionHandleType])) {
    return null;
  }
  ...
  if (CustomConnectionLineComponent) {
    return jsx("g", { className: "react-flow__connection", children: jsx(CustomConnectionLineComponent, { connectionLineType, connectionLineStyle, fromNode, fromHandle, fromX, fromY, toX, toY, fromPosition, toPosition }) });
  }
  let dAttr = "";
  const pathParams = {
    sourceX: fromX,
...
const ArrowSymbol = ({ color: color2 = "none", strokeWidth = 1 }) => {
  return jsx("polyline", { stroke: color2, strokeLinecap: "round", strokeLinejoin: "round", strokeWidth, fill: "none", points: "-5,-4 0,0 -5,4" });
};
const ArrowClosedSymbol = ({ color: color2 = "none", strokeWidth = 1 }) => {
  return jsx("polyline", { stroke: color2, strokeLinecap: "round", strokeLinejoin: "round", strokeWidth, fill: color2, points: "-5,-4 0,0 -5,4 -5,-4" });
};
const MarkerSymbols = {
...
function useMarkerSymbol(type2) {
  const symbol = react.exports.useMemo(() => {
...
const Marker = ({ id: id2, type: type2, color: color2, width = 12.5, height = 12.5, markerUnits = "strokeWidth", strokeWidth, orient = "auto-start-reverse" }) => {
...
const MarkerDefinitions = ({ defaultColor, rfId }) => {
  const markers = useStore(
    react.exports.useCallback(markerSelector({ defaultColor, rfId }), [defaultColor, rfId]),
...
var MarkerDefinitions$1 = react.exports.memo(MarkerDefinitions);
const selector$2 = (s) => ({
...
const EdgeRenderer = (props) => {
  const { connectionNodeId, connectionHandleType, nodesConnectable, edgesFocusable, elementsSelectable, width, height, connectionMode, nodeInternals } = useStore(selector$2, shallow);
  ...
  return jsxs(Fragment, { children: [edgeTree.map(({ level, edges, isMaxLevel }) => jsxs("svg", { style: { zIndex: level }, width, height, className: "react-flow__edges react-flow__container", children: [isMaxLevel && jsx(MarkerDefinitions$1, { defaultColor: defaultMarkerColor, rfId: props.rfId }), jsx("g", { children: edges.map((edge) => {
    ...
    return jsx(EdgeComponent, { id: edge.id, className: cc$1([edge.className, props.noPanClassName]), type: edgeType, data: edge.data, selected: !!edge.selected, animated: !!edge.animated, hidden: !!edge.hidden, label: edge.label, labelStyle: edge.labelStyle, labelShowBg: edge.labelShowBg, labelBgStyle: edge.labelBgStyle, labelBgPadding: edge.labelBgPadding, labelBgBorderRadius: edge.labelBgBorderRadius, style: edge.style, source: edge.source, target: edge.target, sourceHandleId: edge.sourceHandle, targetHandleId: edge.targetHandle, markerEnd: edge.markerEnd, markerStart: edge.markerStart, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, elementsSelectable, onEdgeUpdate: props.onEdgeUpdate, onContextMenu: props.onEdgeContextMenu, onMouseEnter: props.onEdgeMouseEnter, onMouseMove: props.onEdgeMouseMove, onMouseLeave: props.onEdgeMouseLeave, onClick: props.onEdgeClick, edgeUpdaterRadius: props.edgeUpdaterRadius, onEdgeDoubleClick: props.onEdgeDoubleClick, onEdgeUpdateStart: props.onEdgeUpdateStart, onEdgeUpdateEnd: props.onEdgeUpdateEnd, rfId: props.rfId, ariaLabel: edge.ariaLabel, isFocusable, pathOptions: "pathOptions" in edge ? edge.pathOptions : void 0, interactionWidth: edge.interactionWidth }, edge.id);
  }) })] }, level)), renderConnectionLine && jsx("svg", { style: connectionLineContainerStyle, width, height, className: "react-flow__edges react-flow__connectionline react-flow__container", children: jsx(ConnectionLine, { connectionNodeId, connectionHandleType, connectionLineStyle, connectionLineType, isConnectable: nodesConnectable, CustomConnectionLineComponent: connectionLineComponent }) })] });
};
EdgeRenderer.displayName = "EdgeRenderer";
var EdgeRenderer$1 = react.exports.memo(EdgeRenderer);
const selector$1 = (s) => `translate(${s.transform[0]}px,${s.transform[1]}px) scale(${s.transform[2]})`;
function Viewport({ children: children2 }) {
  const transform = useStore(selector$1);
  return jsx("div", { className: "react-flow__viewport react-flow__container", style: { transform }, children: children2 });
}
...
const GraphView = ({ nodeTypes: nodeTypes2, edgeTypes, onMove, onMoveStart, onMoveEnd, onInit, onNodeClick, onEdgeClick, onNodeDoubleClick, onEdgeDoubleClick, onNodeMouseEnter, onNodeMouseMove, onNodeMouseLeave, onNodeContextMenu, onSelectionContextMenu, connectionLineType, connectionLineStyle, connectionLineComponent, connectionLineContainerStyle, selectionKeyCode, multiSelectionKeyCode, zoomActivationKeyCode, deleteKeyCode, onlyRenderVisibleElements, elementsSelectable, selectNodesOnDrag, defaultViewport, translateExtent, minZoom, maxZoom, preventScrolling, defaultMarkerColor, zoomOnScroll, zoomOnPinch, panOnScroll, panOnScrollSpeed, panOnScrollMode, zoomOnDoubleClick, panOnDrag, onPaneClick, onPaneMouseEnter, onPaneMouseMove, onPaneMouseLeave, onPaneScroll, onPaneContextMenu, onEdgeUpdate, onEdgeContextMenu, onEdgeMouseEnter, onEdgeMouseMove, onEdgeMouseLeave, edgeUpdaterRadius, onEdgeUpdateStart, onEdgeUpdateEnd, noDragClassName, noWheelClassName, noPanClassName, elevateEdgesOnSelect, disableKeyboardA11y, nodeOrigin, nodeExtent, rfId }) => {
  ...
  return jsx(FlowRenderer$1, { onPaneClick, onPaneMouseEnter, onPaneMouseMove, onPaneMouseLeave, onPaneContextMenu, onPaneScroll, deleteKeyCode, selectionKeyCode, multiSelectionKeyCode, zoomActivationKeyCode, elementsSelectable, onMove, onMoveStart, onMoveEnd, zoomOnScroll, zoomOnPinch, zoomOnDoubleClick, panOnScroll, panOnScrollSpeed, panOnScrollMode, panOnDrag, defaultViewport, translateExtent, minZoom, maxZoom, onSelectionContextMenu, preventScrolling, noDragClassName, noWheelClassName, noPanClassName, disableKeyboardA11y, children: jsxs(Viewport, { children: [jsx(EdgeRenderer$1, { edgeTypes, onEdgeClick, onEdgeDoubleClick, connectionLineType, connectionLineStyle, connectionLineComponent, connectionLineContainerStyle, onEdgeUpdate, onlyRenderVisibleElements, onEdgeContextMenu, onEdgeMouseEnter, onEdgeMouseMove, onEdgeMouseLeave, onEdgeUpdateStart, onEdgeUpdateEnd, edgeUpdaterRadius, defaultMarkerColor, noPanClassName, elevateEdgesOnSelect: !!elevateEdgesOnSelect, disableKeyboardA11y, rfId }), jsx("div", { className: "react-flow__edgelabel-renderer", id: "edgelabel-portal" }), jsx(NodeRenderer$1, { nodeTypes: nodeTypes2, onNodeClick, onNodeDoubleClick, onNodeMouseEnter, onNodeMouseMove, onNodeMouseLeave, onNodeContextMenu, selectNodesOnDrag, onlyRenderVisibleElements, noPanClassName, noDragClassName, disableKeyboardA11y, nodeOrigin, nodeExtent, rfId })] }) });
};
...
const createRFStore = () => createStore((set2, get2) => ({
  ...initialState,
  setNodes: (nodes) => {
    set2({ nodeInternals: createNodeInternals(nodes, get2().nodeInternals) });
  },
  setEdges: (edges) => {
  ...
  updateNodePositions: (nodeDragItems, positionChanged = true, dragging = false) => {
    const { onNodesChange, nodeInternals, hasDefaultNodes } = get2();
  ...
  addSelectedNodes: (selectedNodeIds) => {
    const { multiSelectionActive, nodeInternals, edges } = get2();
    let changedNodes;
  ...
  addSelectedEdges: (selectedEdgeIds) => {
    const { multiSelectionActive, edges, nodeInternals } = get2();
    let changedEdges;
  ...
  unselectNodesAndEdges: ({ nodes, edges } = {}) => {
    const { nodeInternals, edges: storeEdges } = get2();
  ...
  resetSelectedElements: () => {
    const { nodeInternals, edges } = get2();
    const nodes = Array.from(nodeInternals.values());
...
const ReactFlowProvider = ({ children: children2 }) => {
  const storeRef = react.exports.useRef(null);
  if (!storeRef.current) {
    storeRef.current = createRFStore();
  }
  return jsx(Provider$1, { value: storeRef.current, children: children2 });
};
...
const Wrapper = ({ children: children2 }) => {
  ...
  return jsx(ReactFlowProvider, { children: children2 });
};
Wrapper.displayName = "ReactFlowWrapper";
function useNodeOrEdgeTypes(nodeOrEdgeTypes, createTypes) {
  react.exports.useRef(null);
  const typesParsed = react.exports.useMemo(() => {
...
function $6ed0406888f73fc4$export$43e446d32b3d21af(...refs) {
  return (node2) => refs.forEach(
    (ref2) => $6ed0406888f73fc4$var$setRef(ref2, node2)
  );
}
...
const $5e63c961fc1ce211$export$8c6ed5c666ac1360 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  const childrenArray = react.exports.Children.toArray(children2);
...
const $5e63c961fc1ce211$var$SlotClone = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { children: children2, ...slotProps } = props;
  if (/* @__PURE__ */ react.exports.isValidElement(children2))
    return /* @__PURE__ */ react.exports.cloneElement(children2, {
  ...
  return react.exports.Children.count(children2) > 1 ? react.exports.Children.only(null) : null;
});
$5e63c961fc1ce211$var$SlotClone.displayName = "SlotClone";
const $5e63c961fc1ce211$export$d9f1ccf0bdb05d45 = ({ children: children2 }) => {
  return /* @__PURE__ */ react.exports.createElement(react.exports.Fragment, null, children2);
};
...
const $5cb92bef7577960e$export$177fb62ff3ec1f22 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.div, _extends({}, layerProps, {
    ref: composedRefs,
    style: {
...
function $1746a345f3d73bb7$export$f680877a34711e37(deterministicId) {
  const [id2, setId] = react.exports.useState($1746a345f3d73bb7$var$useReactId());
...
function useLatestRef(value) {
  const ref2 = react.exports.useRef(value);
  index(() => {
    ref2.current = value;
  });
  return ref2;
...
const $7e8f5cd07187803e$export$21b07c8f274aebd5 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.svg, _extends({}, arrowProps, {
    ref: forwardedRef,
    width,
    height,
    viewBox: "0 0 30 10",
    preserveAspectRatio: "none"
  }), props.asChild ? children2 : /* @__PURE__ */ react.exports.createElement("polygon", {
    points: "0,0 30,0 15,10"
  }));
});
const $7e8f5cd07187803e$export$be92b6f5f03c0fe9 = $7e8f5cd07187803e$export$21b07c8f274aebd5;
function $db6c3485150b8e66$export$1ab7ae714698c4b8(element) {
  const [size, setSize] = react.exports.useState(void 0);
  $9f79659886946c16$export$e5c5a5f917a5871c(() => {
    if (element) {
      setSize({
        width: element.offsetWidth,
...
const $cf1ac5d9fe0e8206$export$ecd4e1ccab6ed6d = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return virtualRef ? null : /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.div, _extends({}, anchorProps, {
    ref: composedRefs
  }));
});
...
const $cf1ac5d9fe0e8206$export$bc4ae5855d3c4fc = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  const [arrow$12, setArrow] = react.exports.useState(null);
...
const $cf1ac5d9fe0e8206$export$79d62cd4e10a3fd0 = /* @__PURE__ */ react.exports.forwardRef(function $cf1ac5d9fe0e8206$export$79d62cd4e10a3fd02(props, forwardedRef) {
  ...
  }, /* @__PURE__ */ react.exports.createElement($7e8f5cd07187803e$export$be92b6f5f03c0fe9, _extends({}, arrowProps, {
    ref: forwardedRef,
    style: {
      ...arrowProps.style,
      display: "block"
...
const $f1701beae083dbae$export$602eac185826482c = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return container ? /* @__PURE__ */ $7SXl2$reactdom.createPortal(/* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.div, _extends({}, portalProps, {
    ref: forwardedRef
  })), container) : null;
});
...
const $921a889cee6df7e8$export$99c2b779aa4e8b8b = (props) => {
  ...
  }) : react.exports.Children.only(children2);
...
function useCallbackRef(initialValue, callback) {
  var ref2 = react.exports.useState(function() {
    return {
      value: initialValue,
      callback,
      facade: {
...
var RemoveScroll = react.exports.forwardRef(function(props, parentRef) {
  var ref2 = react.exports.useRef(null);
  ...
  return /* @__PURE__ */ jsxs(Fragment, {
    children: [enabled && /* @__PURE__ */ jsx(SideCar2, {
      ...
      lockRef: ref2
    }), forwardProps ? react.exports.cloneElement(react.exports.Children.only(children2), __assign(__assign({}, containerProps), {
      ref: containerRef
...
function RemoveScrollSideCar(props) {
  ...
  var shouldPrevent = react.exports.useCallback(function(_event) {
    ...
    var sourceEvent2 = shouldPreventQueue.current.filter(function(e2) {
...
var ReactRemoveScroll = react.exports.forwardRef(function(props, ref2) {
...
const $cb5cc270b50c6fcd$export$5b6b19405a83ff9d = (props) => {
  const { __scopePopover, children: children2, open: openProp, defaultOpen, onOpenChange, modal = false } = props;
  const popperScope = $cb5cc270b50c6fcd$var$usePopperScope(__scopePopover);
  const triggerRef = react.exports.useRef(null);
  const [hasCustomAnchor, setHasCustomAnchor] = react.exports.useState(false);
  const [open = false, setOpen] = $71cd76cc60e0454e$export$6f32135080cb4c3({
    prop: openProp,
    defaultProp: defaultOpen,
    onChange: onOpenChange
  });
  return /* @__PURE__ */ react.exports.createElement($cf1ac5d9fe0e8206$export$be92b6f5f03c0fe9, popperScope, /* @__PURE__ */ react.exports.createElement($cb5cc270b50c6fcd$var$PopoverProvider, {
    scope: __scopePopover,
    contentId: $1746a345f3d73bb7$export$f680877a34711e37(),
    triggerRef,
    open,
    onOpenChange: setOpen,
    onOpenToggle: react.exports.useCallback(
      () => setOpen(
        (prevOpen) => !prevOpen
      ),
      [
    ...
    hasCustomAnchor,
    onCustomAnchorAdd: react.exports.useCallback(
      () => setHasCustomAnchor(true),
      []
    ),
    onCustomAnchorRemove: react.exports.useCallback(
...
const $cb5cc270b50c6fcd$export$7dacb05d26466c3 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopePopover, ...triggerProps } = props;
  const context = $cb5cc270b50c6fcd$var$usePopoverContext($cb5cc270b50c6fcd$var$TRIGGER_NAME, __scopePopover);
  const popperScope = $cb5cc270b50c6fcd$var$usePopperScope(__scopePopover);
  const composedTriggerRef = $6ed0406888f73fc4$export$c7b2cbe3552a0d05(forwardedRef, context.triggerRef);
  const trigger = /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.button, _extends({
    type: "button",
  ...
  return context.hasCustomAnchor ? trigger : /* @__PURE__ */ react.exports.createElement($cf1ac5d9fe0e8206$export$b688253958b8dfe7, _extends({
    asChild: true
  }, popperScope), trigger);
});
...
const $cb5cc270b50c6fcd$export$dd679ffb4362d2d4 = (props) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($cb5cc270b50c6fcd$var$PortalProvider, {
    scope: __scopePopover,
    forceMount
...
const $cb5cc270b50c6fcd$export$d7e1f420b25549ff = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($921a889cee6df7e8$export$99c2b779aa4e8b8b, {
    present: forceMount || context.open
  }, context.modal ? /* @__PURE__ */ react.exports.createElement($cb5cc270b50c6fcd$var$PopoverContentModal, _extends({}, contentProps, {
    ref: forwardedRef
  })) : /* @__PURE__ */ react.exports.createElement($cb5cc270b50c6fcd$var$PopoverContentNonModal, _extends({}, contentProps, {
    ref: forwardedRef
  })));
});
const $cb5cc270b50c6fcd$var$PopoverContentModal = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const context = $cb5cc270b50c6fcd$var$usePopoverContext($cb5cc270b50c6fcd$var$CONTENT_NAME, props.__scopePopover);
  const contentRef = react.exports.useRef(null);
  const composedRefs = $6ed0406888f73fc4$export$c7b2cbe3552a0d05(forwardedRef, contentRef);
  const isRightClickOutsideRef = react.exports.useRef(false);
  react.exports.useEffect(() => {
    const content = contentRef.current;
    if (content)
      return hideOthers(content);
  }, []);
  return /* @__PURE__ */ react.exports.createElement($epM9y$RemoveScroll, {
    as: $5e63c961fc1ce211$export$8c6ed5c666ac1360,
    allowPinchZoom: true
  }, /* @__PURE__ */ react.exports.createElement($cb5cc270b50c6fcd$var$PopoverContentImpl, _extends({}, props, {
    ref: composedRefs,
...
const $cb5cc270b50c6fcd$var$PopoverContentNonModal = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const context = $cb5cc270b50c6fcd$var$usePopoverContext($cb5cc270b50c6fcd$var$CONTENT_NAME, props.__scopePopover);
  const hasInteractedOutsideRef = react.exports.useRef(false);
  return /* @__PURE__ */ react.exports.createElement($cb5cc270b50c6fcd$var$PopoverContentImpl, _extends({}, props, {
    ref: forwardedRef,
    trapFocus: false,
...
const $cb5cc270b50c6fcd$var$PopoverContentImpl = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopePopover, trapFocus, onOpenAutoFocus, onCloseAutoFocus, disableOutsidePointerEvents, onEscapeKeyDown, onPointerDownOutside, onFocusOutside, onInteractOutside, ...contentProps } = props;
  const context = $cb5cc270b50c6fcd$var$usePopoverContext($cb5cc270b50c6fcd$var$CONTENT_NAME, __scopePopover);
  const popperScope = $cb5cc270b50c6fcd$var$usePopperScope(__scopePopover);
  $3db38b7d1fb3fe6a$export$b7ece24a22aeda8c();
  return /* @__PURE__ */ react.exports.createElement($d3863c46a17e8a28$export$20e40289641fbbb6, {
    asChild: true,
    loop: true,
    trapped: trapFocus,
  ...
  }, /* @__PURE__ */ react.exports.createElement($cf1ac5d9fe0e8206$export$7c6e2c02157bb7d2, _extends({
    "data-state": $cb5cc270b50c6fcd$var$getState(context.open),
    role: "dialog",
...
const $cb5cc270b50c6fcd$export$d6ac43ebaa40d53e = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopePopover, ...closeProps } = props;
  const context = $cb5cc270b50c6fcd$var$usePopoverContext($cb5cc270b50c6fcd$var$CLOSE_NAME, __scopePopover);
  return /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.button, _extends({
    type: "button"
  }, closeProps, {
    ref: forwardedRef,
...
const $cb5cc270b50c6fcd$export$3152841115e061b2 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopePopover, ...arrowProps } = props;
  const popperScope = $cb5cc270b50c6fcd$var$usePopperScope(__scopePopover);
  return /* @__PURE__ */ react.exports.createElement($cf1ac5d9fe0e8206$export$21b07c8f274aebd5, _extends({}, popperScope, arrowProps, {
    ref: forwardedRef
  }));
});
...
var CheckIcon = /* @__PURE__ */ react.exports.forwardRef(function(_ref, forwardedRef) {
...
var Cross2Icon = /* @__PURE__ */ react.exports.forwardRef(function(_ref, forwardedRef) {
...
var QuestionMarkIcon = /* @__PURE__ */ react.exports.forwardRef(function(_ref, forwardedRef) {
  var _ref$color = _ref.color, color2 = _ref$color === void 0 ? "currentColor" : _ref$color, props = _objectWithoutPropertiesLoose(_ref, _excluded$3z);
  return react.exports.createElement("svg", Object.assign({
    width: "15",
    height: "15",
    viewBox: "0 0 15 15",
...
function Popover({
  ...
  return /* @__PURE__ */ jsxs($cb5cc270b50c6fcd$export$be92b6f5f03c0fe9, {
    ...props,
    children: [/* @__PURE__ */ jsx($cb5cc270b50c6fcd$export$41fb9f06171c75f4, {
      asChild: true,
      children: children2
    }), /* @__PURE__ */ jsx($cb5cc270b50c6fcd$export$602eac185826482c, {
      children: /* @__PURE__ */ jsxs($cb5cc270b50c6fcd$export$7c6e2c02157bb7d2, {
        className: "relative w-64 px-4 py-2 text-sm text-white rounded-lg shadow bg-neutral-700",
        side: side || "top",
        sideOffset: 5,
        children: [/* @__PURE__ */ jsx("div", {
          className: "mt-6",
          children: content
        }), /* @__PURE__ */ jsx($cb5cc270b50c6fcd$export$f39c2d165cd861fe, {
          className: "absolute top-0 right-0 p-1 m-2 transition rounded-full hover:bg-neutral-600",
          "aria-label": "Close",
          children: /* @__PURE__ */ jsx(Cross2Icon, {})
        }), /* @__PURE__ */ jsx($cb5cc270b50c6fcd$export$21b07c8f274aebd5, {
          className: "shadow fill-neutral-700"
        })]
      })
    })]
  });
}
const distribute = (min2, max2, n2, i) => min2 + (i + 1) * (max2 - min2) / (n2 + 1);
...
function BaseNode$1({
  ...
  return /* @__PURE__ */ jsxs("div", {
    className: "relative flex flex-col w-32 font-sans group",
    children: [!standalone && inputs2.map((id2, i) => /* @__PURE__ */ jsx(Handle$1, {
      id: id2 ? `in-${id2}` : "in",
      title: id2 || "in",
      position: Position.Left,
      style: {
        top: distribute(0, 100, inputs2.length, i) + "%"
      },
      type: "target"
    }, id2)), /* @__PURE__ */ jsxs("div", {
      className: "drag flex justify-between items-center absolute opacity-80 group-hover:opacity-100 -translate-y-[45%] transition group-hover:-translate-y-[80%] rounded-t-xl w-full px-2 pb-2 bg-gradient-to-r from-pink-500 to-pink-400",
      children: [/* @__PURE__ */ jsx("span", {
        className: "font-bold text-white font-title",
        children: name2
      }), info2 && /* @__PURE__ */ jsx(Popover, {
        content: info2,
        side: "right",
        children: /* @__PURE__ */ jsx("button", {
          "aria-label": "Info",
          children: /* @__PURE__ */ jsx(QuestionMarkIcon, {
            className: "w-5 h-5 p-1 text-white transition rounded-full opacity-0 group-hover:opacity-100 hover:bg-neutral-700"
          })
        })
      })]
    }), /* @__PURE__ */ jsx("div", {
      ...
      children: children2
    }), !standalone && outputs2.map((id2, i) => /* @__PURE__ */ jsx(Handle$1, {
      id: id2 ? `out-${id2}` : "out",
      title: id2 || "out",
      position: Position.Right,
      style: {
        top: distribute(0, 100, outputs2.length, i) + "%"
      },
      type: "source"
    }, id2))]
  });
...
function AddNode({
  ...
  return /* @__PURE__ */ jsx(BaseNode$1, {
    name: name$8,
    info: info$8,
    inputs: inputs$8,
    outputs: outputs$8
...
const asReactFlowNode$8 = (id2, data = defaults$8, opts = {}) => ({
  type: AddNode.name,
  id: id2,
  data,
  ...opts
});
const asAudioNodes$8 = (id2, data = defaults$8, connections = {}) => {
  var _a;
  return [keyed(id2 + "a", node("GainNode", [], [ref(id2)])), keyed(id2 + "b", node("GainNode", [], [ref(id2)])), keyed(id2, node("GainNode", [param("gain", 0)], (_a = connections[id2]) != null ? _a : []))];
};
const Add = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  defaults: defaults$8,
  default: AddNode,
  asReactFlowNode: asReactFlowNode$8,
  asAudioNodes: asAudioNodes$8
}, Symbol.toStringTag, { value: "Module" }));
const ctx = new AudioContext();
function useAnalyser(id2) {
  const analyser = react.exports.useMemo(() => new AnalyserNode(ctx, { fftSize: 2048 }), []);
  react.exports.useEffect(() => {
    if (id2 in ctx.nodes) {
      ctx.nodes[id2].connect(analyser);
      return () => {
        var _a;
        return (_a = ctx.nodes[id2]) == null ? void 0 : _a.disconnect(analyser);
      };
    }
  }, []);
  return analyser;
...
function useAnimationFrame(callback) {
  const previousTimeRef = react.exports.useRef(null);
  const requestRef = react.exports.useRef(null);
  const animate = react.exports.useCallback(
    (time) => {
...
function Oscilloscope({
  ...
  useAnimationFrame(() => {
    ...
    const height2 = canvas.current.height;
    const width2 = canvas.current.width;
    const context = canvas.current.getContext("2d");
    const sliceWidth = width2 * 1 / data.length;
    context.lineWidth = 3;
    context.strokeStyle = "#ec4899";
    context.clearRect(0, 0, width2, height2);
    context.beginPath();
    context.moveTo(0, height2 / 2);
    const x2 = data.reduce((x22, n2) => {
      const y2 = n2 / 255 * height2;
      context.lineTo(x22, y2);
      return x22 + sliceWidth;
    }, 0);
    context.lineTo(x2, height2 / 2);
    context.stroke();
  });
...
const $b73a6c6685e72184$export$b04be29aa201d4f5 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  return /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.label, _extends({}, props, {
    ref: forwardedRef,
    onMouseDown: (event) => {
...
function $010c2913dbd2fe3d$export$5cae361ad82dce8b(value) {
  const ref2 = react.exports.useRef({
    value,
    previous: value
  });
  return react.exports.useMemo(() => {
...
const $faa2e61a3361514f$export$472062a354075cee = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { name: name2, min: min2 = 0, max: max2 = 100, step = 1, orientation = "horizontal", disabled = false, minStepsBetweenThumbs = 0, defaultValue = [
    min2
  ], value: value1, onValueChange = () => {
  }, onValueCommit = () => {
  }, inverted = false, ...sliderProps } = props;
  const [slider, setSlider] = react.exports.useState(null);
  const composedRefs = $6ed0406888f73fc4$export$c7b2cbe3552a0d05(
    forwardedRef,
    (node2) => setSlider(node2)
  );
  const thumbRefs = react.exports.useRef(/* @__PURE__ */ new Set());
  const valueIndexToChangeRef = react.exports.useRef(0);
  ...
  const valuesBeforeSlideStartRef = react.exports.useRef(values);
  ...
  return /* @__PURE__ */ react.exports.createElement($faa2e61a3361514f$var$SliderProvider, {
    ...
    values,
    orientation
  }, /* @__PURE__ */ react.exports.createElement($faa2e61a3361514f$var$Collection.Provider, {
    scope: props.__scopeSlider
  }, /* @__PURE__ */ react.exports.createElement($faa2e61a3361514f$var$Collection.Slot, {
    scope: props.__scopeSlider
  }, /* @__PURE__ */ react.exports.createElement(SliderOrientation, _extends({
    ...
    (value, index2) => /* @__PURE__ */ react.exports.createElement($faa2e61a3361514f$var$BubbleInput, {
      key: index2,
      name: name2 ? name2 + (values.length > 1 ? "[]" : "") : void 0,
      value
    })
  ));
});
...
const $faa2e61a3361514f$var$SliderHorizontal = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  const rectRef = react.exports.useRef();
  ...
  return /* @__PURE__ */ react.exports.createElement($faa2e61a3361514f$var$SliderOrientationProvider, {
    scope: props.__scopeSlider,
    startEdge: isSlidingFromLeft ? "left" : "right",
    endEdge: isSlidingFromLeft ? "right" : "left",
    direction: isSlidingFromLeft ? 1 : -1,
    size: "width"
  }, /* @__PURE__ */ react.exports.createElement($faa2e61a3361514f$var$SliderImpl, _extends({
    dir: direction,
...
const $faa2e61a3361514f$var$SliderVertical = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { min: min2, max: max2, inverted, onSlideStart, onSlideMove, onSlideEnd, onStepKeyDown, ...sliderProps } = props;
  const sliderRef = react.exports.useRef(null);
  const ref2 = $6ed0406888f73fc4$export$c7b2cbe3552a0d05(forwardedRef, sliderRef);
  const rectRef = react.exports.useRef();
...
const $faa2e61a3361514f$var$SliderImpl = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopeSlider, onSlideStart, onSlideMove, onSlideEnd, onHomeKeyDown, onEndKeyDown, onStepKeyDown, ...sliderProps } = props;
  const context = $faa2e61a3361514f$var$useSliderContext($faa2e61a3361514f$var$SLIDER_NAME, __scopeSlider);
  return /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.span, _extends({}, sliderProps, {
    ref: forwardedRef,
...
const $faa2e61a3361514f$export$105594979f116971 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopeSlider, ...trackProps } = props;
  const context = $faa2e61a3361514f$var$useSliderContext($faa2e61a3361514f$var$TRACK_NAME, __scopeSlider);
  return /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.span, _extends({
...
const $faa2e61a3361514f$export$a5cf38a7a000fe77 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
...
const $faa2e61a3361514f$export$2c1b491743890dec = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const getItems = $faa2e61a3361514f$var$useCollection(props.__scopeSlider);
  const [thumb, setThumb] = react.exports.useState(null);
  const composedRefs = $6ed0406888f73fc4$export$c7b2cbe3552a0d05(
    forwardedRef,
  ...
  return /* @__PURE__ */ react.exports.createElement($faa2e61a3361514f$var$SliderThumbImpl, _extends({}, props, {
    ref: composedRefs,
    index: index2
  }));
});
const $faa2e61a3361514f$var$SliderThumbImpl = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  }, /* @__PURE__ */ react.exports.createElement($faa2e61a3361514f$var$Collection.ItemSlot, {
    scope: props.__scopeSlider
  }, /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.span, _extends({
    role: "slider",
...
const $faa2e61a3361514f$var$BubbleInput = (props) => {
  ...
  return /* @__PURE__ */ react.exports.createElement("input", _extends({
    style: {
      display: "none"
    }
  }, inputProps, {
...
function Slider({
  ...
  max: max2 = 100,
  step = 1,
  defaultValue = mean(min2, max2),
  onValueChange,
  formatter,
  ...props
}) {
  var _a, _b;
  return /* @__PURE__ */ jsxs($b73a6c6685e72184$export$be92b6f5f03c0fe9, {
    ...props,
    children: [/* @__PURE__ */ jsx("span", {
      className: "text-xs font-bold",
      children: label
    }), /* @__PURE__ */ jsxs($faa2e61a3361514f$export$be92b6f5f03c0fe9, {
      ...
      onValueChange,
      "aria-label": label,
      children: [/* @__PURE__ */ jsx($faa2e61a3361514f$export$13921ac0cc260818, {
        className: "relative h-1 rounded grow bg-neutral-700",
        children: /* @__PURE__ */ jsx($faa2e61a3361514f$export$9a58ef0d7ad3278c, {
          className: "absolute h-full rounded bg-gradient-to-r from-pink-400 to-pink-500"
        })
      }), /* @__PURE__ */ jsx($faa2e61a3361514f$export$6521433ed15a34db, {
        className: "block w-4 h-4 bg-white rounded shadow-lg"
      })]
    }), /* @__PURE__ */ jsx("span", {
      className: "block text-xs text-right",
      children: (_b = (_a = formatter == null ? void 0 : formatter(value != null ? value : defaultValue)) != null ? _a : value) != null ? _b : defaultValue
    })]
  });
}
const GraphContext = react.exports.createContext(null);
function useGraphStore(...args) {
  return react.exports.useContext(GraphContext)(...args);
}
const name$7 = "amp";
const info$7 = /* @__PURE__ */ jsxs("p", {
  children: ["An amp modifies the volume of a signal by multiplying it by a ", /* @__PURE__ */ jsx("strong", {
    children: "gain"
  }), " value. A gain ", /* @__PURE__ */ jsx("code", {
    children: "0.5"
  }), " will reduce the volume by half, while a gain of ", /* @__PURE__ */ jsx("code", {
    children: "2"
  }), " will double it."]
});
const inputs$7 = ["", ".gain"];
...
function AmpNode({
  ...
  return /* @__PURE__ */ jsxs(BaseNode$1, {
    name: name$7,
    info: info$7,
    inputs: inputs$7,
    outputs: outputs$7,
    children: [/* @__PURE__ */ jsx(Slider, {
      ...
      max: 2,
      step: 0.01,
      onValueChange: ([val]) => updateNode(id2, {
        ...data,
        gain: val
      })
    }), /* @__PURE__ */ jsx(Oscilloscope, {
      nodeId: id2,
      className: "w-full h-3 my-2 transition-all group-hover:h-8"
    })]
  });
}
const type$4 = AmpNode.name;
const asReactFlowNode$7 = (id2, data = defaults$7, opts = {}) => ({
  type: type$4,
  id: id2,
  data,
  ...opts
});
const asAudioNodes$7 = (id2, data = defaults$7, connections = {}) => {
  var _a;
  return [keyed(id2, node("GainNode", [param("gain", data.gain)], (_a = connections == null ? void 0 : connections[id2]) != null ? _a : []))];
};
const Amp = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  defaults: defaults$7,
  default: AmpNode,
  type: type$4,
  asReactFlowNode: asReactFlowNode$7,
  asAudioNodes: asAudioNodes$7
}, Symbol.toStringTag, { value: "Module" }));
const name$6 = "const";
const info$6 = "";
const inputs$6 = [];
...
const asReactFlowNode$6 = (id2, data = defaults$6, opts = {}) => ({
  type: ConstNode.name,
  id: id2,
  data,
  ...opts
});
const asAudioNodes$6 = (id2, data = defaults$6, connections = {}) => {
  var _a;
  return [node("ConstantSourceNode", [param("offset", data.value)], (_a = connections[id2]) != null ? _a : [])];
};
const Const = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  defaults: defaults$6,
  default: ConstNode,
  asReactFlowNode: asReactFlowNode$6,
  asAudioNodes: asAudioNodes$6
}, Symbol.toStringTag, { value: "Module" }));
const name$5 = "dac";
const info$5 = /* @__PURE__ */ jsx("p", {});
const inputs$5 = [""];
const outputs$5 = [];
const defaults$5 = {
  gain: 0
...
const type$3 = DacNode.name;
const asReactFlowNode$5 = (id2, data = defaults$5, opts = {}) => ({
  type: type$3,
  id: id2,
  data,
  ...opts
});
const asAudioNodes$5 = (id2, data = defaults$5) => [keyed(id2, node("GainNode", [param("gain", data.gain)], [node("AudioDestinationNode")]))];
const Dac = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  defaults: defaults$5,
  default: DacNode,
  type: type$3,
  asReactFlowNode: asReactFlowNode$5,
  asAudioNodes: asAudioNodes$5
}, Symbol.toStringTag, { value: "Module" }));
const name$4 = "mul";
const info$4 = "";
const inputs$4 = ["", ".gain"];
...
const asReactFlowNode$4 = (id2, data = defaults$4, opts = {}) => ({
  type: MulNode$1.name,
  id: id2,
  data,
  ...opts
});
const asAudioNodes$4 = (id2, data = defaults$4, connections = {}) => {
  var _a;
  return [keyed(id2, node("GainNode", [param("gain", 0)], (_a = connections[id2]) != null ? _a : []))];
};
const MulNode$2 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  defaults: defaults$4,
  default: MulNode$1,
  asReactFlowNode: asReactFlowNode$4,
  asAudioNodes: asAudioNodes$4
}, Symbol.toStringTag, { value: "Module" }));
...
const $d7bdfb9eb0fdf311$export$8699f7c8af148338 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  return /* @__PURE__ */ react.exports.createElement($d7bdfb9eb0fdf311$var$Collection.Provider, {
    ...
    scope: props.__scopeRovingFocusGroup
  }, /* @__PURE__ */ react.exports.createElement($d7bdfb9eb0fdf311$var$RovingFocusGroupImpl, _extends({}, props, {
    ref: forwardedRef
  }))));
});
const $d7bdfb9eb0fdf311$var$RovingFocusGroupImpl = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  const [focusableItemsCount, setFocusableItemsCount] = react.exports.useState(0);
  react.exports.useEffect(() => {
  ...
  return /* @__PURE__ */ react.exports.createElement($d7bdfb9eb0fdf311$var$RovingFocusProvider, {
    ...
    loop,
    currentTabStopId,
    onItemFocus: react.exports.useCallback(
      (tabStopId) => setCurrentTabStopId(tabStopId),
      [
        setCurrentTabStopId
      ]
    ),
    onItemShiftTab: react.exports.useCallback(
      () => setIsTabbingBackOut(true),
      []
    ),
    onFocusableItemAdd: react.exports.useCallback(
      () => setFocusableItemsCount(
        (prevCount) => prevCount + 1
      ),
      []
    ),
    onFocusableItemRemove: react.exports.useCallback(
      () => setFocusableItemsCount(
        (prevCount) => prevCount - 1
      ),
      []
...
const $ce77a8961b41be9e$export$d7b12c4107be0d61 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopeRadio, name: name2, checked = false, required, disabled, value = "on", onCheck, ...radioProps } = props;
  const [button, setButton] = react.exports.useState(null);
  const composedRefs = $6ed0406888f73fc4$export$c7b2cbe3552a0d05(
    forwardedRef,
    (node2) => setButton(node2)
  );
  const hasConsumerStoppedPropagationRef = react.exports.useRef(false);
  const isFormControl = button ? Boolean(button.closest("form")) : true;
  return /* @__PURE__ */ react.exports.createElement($ce77a8961b41be9e$var$RadioProvider, {
    scope: __scopeRadio,
    checked,
    disabled
  }, /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.button, _extends({
    type: "button",
    role: "radio",
  ...
  })), isFormControl && /* @__PURE__ */ react.exports.createElement($ce77a8961b41be9e$var$BubbleInput, {
    control: button,
...
const $ce77a8961b41be9e$export$d35a9ffa9a04f9e7 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
...
const $f99a8c78507165f7$export$a98f0dcb43a68a25 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($f99a8c78507165f7$var$RadioGroupProvider, {
  ...
  }, /* @__PURE__ */ react.exports.createElement($d7bdfb9eb0fdf311$export$be92b6f5f03c0fe9, _extends({
    asChild: true
...
const $f99a8c78507165f7$export$9f866c100ef519e4 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  const isArrowKeyPressedRef = react.exports.useRef(false);
  react.exports.useEffect(() => {
    const handleKeyDown = (event) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($d7bdfb9eb0fdf311$export$6d08773d2e66f8f2, _extends({
    asChild: true
  }, rovingFocusGroupScope, {
    focusable: !isDisabled,
    active: checked
  }), /* @__PURE__ */ react.exports.createElement($ce77a8961b41be9e$export$d7b12c4107be0d61, _extends({
    disabled: isDisabled,
...
const $f99a8c78507165f7$export$5fb54c671a65c88 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopeRadioGroup, ...indicatorProps } = props;
  const radioScope = $f99a8c78507165f7$var$useRadioScope(__scopeRadioGroup);
  return /* @__PURE__ */ react.exports.createElement($ce77a8961b41be9e$export$d35a9ffa9a04f9e7, _extends({}, radioScope, indicatorProps, {
    ref: forwardedRef
  }));
});
...
function Radio({
  ...
  return /* @__PURE__ */ jsxs("div", {
    ...
    children: [/* @__PURE__ */ jsx("span", {
      ...
      children: label
    }), /* @__PURE__ */ jsx($f99a8c78507165f7$export$be92b6f5f03c0fe9, {
...
function RadioItem({
  ...
  return /* @__PURE__ */ jsxs($b73a6c6685e72184$export$be92b6f5f03c0fe9, {
    ...
    ...props,
    children: [/* @__PURE__ */ jsx($f99a8c78507165f7$export$6d08773d2e66f8f2, {
      className: "w-4 h-4 border rounded-full bg-neutral-50 border-neutral-200",
      value,
      children: /* @__PURE__ */ jsx($f99a8c78507165f7$export$adb584737d712b70, {
        className: " flex items-center justify-center w-full h-full relative  after:content-[''] after:block after:w-2 after:h-2 after:rounded-full after:bg-pink-500"
      })
    }), /* @__PURE__ */ jsx("span", {
...
const name$3 = "osc";
const info$3 = /* @__PURE__ */ jsxs(Fragment, {
  children: [/* @__PURE__ */ jsxs("p", {
    children: ["An oscillator produces signals by quickly repeating a ", /* @__PURE__ */ jsx("strong", {
      children: "waveform"
    }), ". The ", /* @__PURE__ */ jsx("strong", {
      children: "frequency"
    }), " of the oscillator, measured in ", /* @__PURE__ */ jsx("strong", {
      children: "Hertz (Hz)"
    }), ", is the number of times it repeats per second."]
  }), /* @__PURE__ */ jsx("p", {
    className: "mt-4 text-right",
    children: /* @__PURE__ */ jsxs(Link, {
      to: "/",
      className: "underline",
      children: ["Learn more about ", /* @__PURE__ */ jsx("strong", {
        children: "pitch"
      }), "."]
    })
  })]
});
const inputs$3 = [".frequency"];
const outputs$3 = [""];
const defaults$3 = {
  frequency: 440,
  waveform: "sine"
...
function OscNode({
  ...
  return /* @__PURE__ */ jsxs(BaseNode$1, {
    ...
    children: [/* @__PURE__ */ jsx(Slider, {
      ...
      className: "my-2 border-neutral-200"
    }), /* @__PURE__ */ jsx(Radio, {
      label: "waveform",
      value: data.waveform,
...
const type$2 = OscNode.name;
const asReactFlowNode$3 = (id2, data = defaults$3, opts = {}) => ({
  type: type$2,
  id: id2,
  data,
  ...opts
});
const asAudioNodes$3 = (id2, data = defaults$3, connections = {}) => {
  var _a;
  return [keyed(id2, node("OscillatorNode", [param("frequency", data.frequency), property("type", data.waveform)], (_a = connections == null ? void 0 : connections[id2]) != null ? _a : []))];
};
const Osc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  defaults: defaults$3,
  default: OscNode,
  type: type$2,
  asReactFlowNode: asReactFlowNode$3,
  asAudioNodes: asAudioNodes$3
}, Symbol.toStringTag, { value: "Module" }));
const name$2 = "xy";
const info$2 = /* @__PURE__ */ jsx("p", {
  children: "An XY pad is a two-dimensional control that can be connected to multiple parameters at once. It's a lot easier to create interesting sounds with this than with a bunch of sliders!"
});
const inputs$2 = [];
const outputs$2 = ["x", "y"];
const defaults$2 = {
...
function XYNode({
  ...
  const svg = react.exports.useRef(null);
  const updateNode = useGraphStore((state) => state.updateNode);
  const [trackingMouse, shouldTrackMouse] = react.exports.useState(false);
  const updateNodePosition = (xPos, yPos) => {
    const {
      width,
      height
    } = svg.current.getBoundingClientRect();
    const x2 = Math.max(0, Math.min(1, xPos / width));
    const y2 = Math.max(0, Math.min(1, yPos / height));
    updateNode(id2, {
      x: x2,
      y: y2
    });
  ...
  react.exports.useEffect(() => {
    ...
    (_a = svg.current) == null ? void 0 : _a.addEventListener("mousedown", onMouseDown);
    (_b = svg.current) == null ? void 0 : _b.addEventListener("mouseup", onMouseUp);
    (_c = svg.current) == null ? void 0 : _c.addEventListener("mousemove", onMouseMove);
    (_d = svg.current) == null ? void 0 : _d.addEventListener("mouseleave", onMouseLeave);
    return () => {
      var _a2, _b2, _c2, _d2;
      (_a2 = svg.current) == null ? void 0 : _a2.removeEventListener("mousedown", onMouseDown);
      (_b2 = svg.current) == null ? void 0 : _b2.removeEventListener("mouseup", onMouseUp);
      (_c2 = svg.current) == null ? void 0 : _c2.removeEventListener("mousemove", onMouseMove);
      (_d2 = svg.current) == null ? void 0 : _d2.removeEventListener("mouseleave", onMouseLeave);
    };
  }, [trackingMouse]);
  return /* @__PURE__ */ jsxs(BaseNode$1, {
    name: name$2,
...
const type$1 = XYNode.name;
const asReactFlowNode$2 = (id2, data = defaults$2, opts = {}) => ({
  type: XYNode.name,
  id: id2,
  data,
  ...opts
});
const asAudioNodes$2 = (id2, data = defaults$2, connections = {}) => {
  var _a, _b;
  return [keyed(id2 + "x", node("ConstantSourceNode", [param("offset", data.x)], (_a = connections == null ? void 0 : connections[id2 + "x"]) != null ? _a : [])), keyed(id2 + "y", node("ConstantSourceNode", [param("offset", data.y)], (_b = connections == null ? void 0 : connections[id2 + "y"]) != null ? _b : []))];
};
const XY = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  default: XYNode,
  type: type$1,
  asReactFlowNode: asReactFlowNode$2,
  asAudioNodes: asAudioNodes$2
}, Symbol.toStringTag, { value: "Module" }));
...
const $e698a72e93240346$export$48513f6b9f8ce62d = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($e698a72e93240346$var$CheckboxProvider, {
  ...
  })), isFormControl && /* @__PURE__ */ react.exports.createElement($e698a72e93240346$var$BubbleInput, {
    control: button,
    bubbles: !hasConsumerStoppedPropagationRef.current,
    name: name2,
    value,
...
const $e698a72e93240346$export$59aad738f51d1c05 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
...
function Checkbox({
  ...
  return /* @__PURE__ */ jsx($e698a72e93240346$export$be92b6f5f03c0fe9, {
    checked,
    onCheckedChange,
    ...props,
    className: "w-4 h-4 border rounded bg-neutral-50 border-neutral-200 mx-2 my-1",
...
const name$1 = "sample";
const info$1 = /* @__PURE__ */ jsx("p", {
  children: "A node that can hold an audio sample"
});
const inputs$1 = [];
const outputs$1 = [""];
const defaults$1 = {
...
function SampleNode({
  ...
  const updateNode = useGraphStore((state) => state.updateNode);
  const onSampleChange = react.exports.useCallback(async (ev) => {
  ...
  const onLoopChange = react.exports.useCallback((newVal) => updateNode(id2, {
    loop: newVal
  }), []);
  return /* @__PURE__ */ jsxs(BaseNode$1, {
    name: name$1,
    ...
    children: [/* @__PURE__ */ jsxs("label", {
      className: "bg-neutral-200 rounded-full py-1 px-2 mb-2",
      children: ["Pick sample", /* @__PURE__ */ jsx("input", {
        className: "hidden",
        type: "file",
        accept: "audio/*",
...
const type = SampleNode.name;
const asReactFlowNode$1 = (id2, data = defaults$1, opts = {}) => ({
  type,
  id: id2,
  data,
  ...opts
});
const asAudioNodes$1 = (id2, data = defaults$1, connections = {}) => {
  var _a;
  return [keyed(`${id2}-${data.version}`, node("AudioBufferSourceNode", [{
    type: "NodeProperty",
    label: "buffer",
    value: data.sample
  }, {
    ...
    label: "loop",
    value: data.loop
  }], (_a = connections == null ? void 0 : connections[id2]) != null ? _a : []))];
};
const Sample = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  default: SampleNode,
  type,
  asReactFlowNode: asReactFlowNode$1,
  asAudioNodes: asAudioNodes$1
}, Symbol.toStringTag, { value: "Module" }));
const nodeTypes = {
  AddNode,
  AmpNode,
...
const asReactFlowNode = (id2, data = defaults, opts = {}) => ({
  type: MulNode.name,
  id: id2,
  data,
  ...opts
});
const asAudioNodes = (id2, data = defaults, connections = {}) => {
  var _a;
  return [keyed(id2, node("GainNode", [param("gain", 0)], (_a = connections[id2]) != null ? _a : []))];
};
const Mul = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  defaults,
  default: MulNode,
  asReactFlowNode,
...
function useAudio() {
  const vctx = react.exports.useMemo(() => new VirtualAudioContext(ctx), []);
  ...
  for (const rfEdge of rfEdges) {
    if (rfEdge.source == rfNode.id) {
      ...
      const connection = ref(
        target.startsWith(".") || target == "" ? rfEdge.target + target : rfEdge.target + target
      );
...
function useElementSize(ref2) {
  var _a, _b;
  const [size, setSize] = react.exports.useState({
    width: 0,
    height: 0
  });
  const handleSize = react.exports.useCallback(() => {
    var _a2, _b2;
    setSize({
      width: ((_a2 = ref2 == null ? void 0 : ref2.current) == null ? void 0 : _a2.offsetWidth) || 0,
      height: ((_b2 = ref2 == null ? void 0 : ref2.current) == null ? void 0 : _b2.offsetHeight) || 0
    });
  }, []);
  react.exports.useLayoutEffect(handleSize, [
    (_a = ref2 == null ? void 0 : ref2.current) == null ? void 0 : _a.offsetHeight,
    (_b = ref2 == null ? void 0 : ref2.current) == null ? void 0 : _b.offsetWidth
  ]);
  return size;
}
function useGraph({ nodes, edges, ref: ref2 }) {
  let useStoreRef = react.exports.useRef(null);
  if (!useStoreRef.current) {
    useStoreRef.current = create$1((set2, get2) => ({
      nodes: nodes.map((n2) => ({
        ...n2,
        position: { x: 0, y: 0, ...n2.position },
        data: {
          ...Nodes[console.log(n2.type), n2.type].defaults,
          ...n2.data
        }
      })),
      edges,
      distributeNodes({ width: width2, height: height2 }) {
        return set2({
          nodes: applyNodeChanges(
            get2().nodes.map(({ id: id2 }, i) => ({
              id: id2,
              type: "position",
              position: {
                x: distribute(0, width2, 3, i) - 54,
                y: height2 / 4
              }
            })),
            get2().nodes
      ...
      updateNode(id2, data) {
        return set2({
          nodes: get2().nodes.map(
            (n2) => n2.id === id2 ? { ...n2, data: { ...n2.data, ...data } } : n2
          )
        });
      },
      removeNode(id2) {
        return set2({ nodes: get2().nodes.filter((n2) => n2.id !== id2) });
      },
      updateNodes(changes) {
        return set2({ nodes: applyNodeChanges(changes, get2().nodes) });
      },
      updateEdges(changes) {
        return set2({ edges: applyEdgeChanges(changes, get2().edges) });
      },
      onConnect(params) {
        return set2({ edges: addEdge(params, get2().edges) });
      }
    }));
  }
  const useStore2 = useStoreRef.current;
  const reactiveNodes = useStore2((state) => state.nodes);
  const reactiveEdges = useStore2((state) => state.edges);
  ...
  const distributeNodes = useStore2((store) => store.distributeNodes);
  const { width, height } = useElementSize(ref2);
  react.exports.useLayoutEffect(() => distributeNodes({ width, height }), [width, height]);
  return useStore2;
}
function Index() {
  return /* @__PURE__ */ jsxs(Fragment, {
    children: [/* @__PURE__ */ jsxs("section", {
      className: "pt-24 pb-16 space-y-16 prose prose-xl bg-neutral-50 max-w-none",
      children: [/* @__PURE__ */ jsx("h1", {
        className: "font-bold text-8xl font-title",
        children: "Let's make some sound!"
      }), /* @__PURE__ */ jsx("p", {
        children: 'bleep.cafe is a place to learn and explore the world of digital and modular synthesis. It runs entirely in your browser so there"s no need to install anything!'
      }), /* @__PURE__ */ jsx("div", {
        className: "w-full h-96 not-prose",
        children: /* @__PURE__ */ jsx(DemoGraph, {})
      }), /* @__PURE__ */ jsxs("p", {
        children: ["If you're entirely new to synthesis or digital audio, we have a ", /* @__PURE__ */ jsx(Link, {
          to: "/play",
          children: "bunch of tutorials"
        }), " that will guide you from the ground up and teach you the basics of synthesis."]
      }), /* @__PURE__ */ jsxs("p", {
        children: ["If you just want to get stuck in and have a play around on the platform, check out the ", /* @__PURE__ */ jsx(Link, {
          to: "/play",
          children: "playground"
        }), "."]
      }), /* @__PURE__ */ jsx("h2", {
        ...
        children: "The easiest way to help out is to start making things and tell all your friends. If you're a developer, you can get involved by reporting any bugs you find on GitHub or even contributing to the codebase yourself: bleep.cafe is entirely open source!"
      })]
    }), /* @__PURE__ */ jsx("footer", {
      className: "p-4 font-mono text-right text-white bg-neutral-900",
      children: /* @__PURE__ */ jsxs("p", {
...
function DemoGraph() {
  ...
  const nodes = useStore2((state) => state.nodes);
  const edges = useStore2((state) => state.edges);
  const updateNodes = useStore2((state) => state.updateNodes);
  const updateEdges = useStore2((state) => state.updateEdges);
...
const $6cc32821e9371a1c$export$d9b273488cd8ce6f = (props) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($cf1ac5d9fe0e8206$export$be92b6f5f03c0fe9, popperScope, /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$var$MenuProvider, {
    ...
    scope: __scopeMenu,
    onClose: react.exports.useCallback(
...
const $6cc32821e9371a1c$export$9fa5ebd18bee4d43 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  const popperScope = $6cc32821e9371a1c$var$usePopperScope(__scopeMenu);
  return /* @__PURE__ */ react.exports.createElement($cf1ac5d9fe0e8206$export$b688253958b8dfe7, _extends({}, popperScope, anchorProps, {
    ref: forwardedRef
  }));
});
...
const $6cc32821e9371a1c$export$479f0f2f71193efe = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$var$Collection.Provider, {
    ...
    scope: props.__scopeMenu
  }, rootContext.modal ? /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$var$MenuRootContentModal, _extends({}, contentProps, {
    ref: forwardedRef
  })) : /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$var$MenuRootContentNonModal, _extends({}, contentProps, {
    ref: forwardedRef
  })))));
});
const $6cc32821e9371a1c$var$MenuRootContentModal = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$var$MenuContentImpl, _extends({}, props, {
    ref: composedRefs,
    trapFocus: context.open,
...
const $6cc32821e9371a1c$var$MenuContentImpl = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  const ScrollLockWrapper = disableOutsideScroll ? $epM9y$RemoveScroll : react.exports.Fragment;
...
const $6cc32821e9371a1c$export$2ce376c2cc3355c8 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  return /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$var$MenuItemImpl, _extends({}, itemProps, {
    ref: composedRefs,
    disabled,
...
const $6cc32821e9371a1c$var$MenuItemImpl = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  }, /* @__PURE__ */ react.exports.createElement($d7bdfb9eb0fdf311$export$6d08773d2e66f8f2, _extends({
    asChild: true
  }, rovingFocusGroupScope, {
    focusable: !disabled
  }), /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.div, _extends({
...
const $6cc32821e9371a1c$export$1cec7dcdd713e220 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
...
const $1b0217ee4a91d156$export$8dc6765e8be191c7 = (props) => {
  ...
  const [open1, setOpen] = react.exports.useState(false);
  ...
  const handleOpenChange = react.exports.useCallback((open) => {
    setOpen(open);
    handleOpenChangeProp(open);
  }, [
    handleOpenChangeProp
  ]);
  return /* @__PURE__ */ react.exports.createElement($1b0217ee4a91d156$var$ContextMenuProvider, {
    scope: __scopeContextMenu,
    open: open1,
    onOpenChange: handleOpenChange,
    modal
  }, /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$export$be92b6f5f03c0fe9, _extends({}, menuScope, {
    dir,
    open: open1,
    onOpenChange: handleOpenChange,
    modal
...
const $1b0217ee4a91d156$export$be6fc7b1d5b0beb9 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  ...
  const menuScope = $1b0217ee4a91d156$var$useMenuScope(__scopeContextMenu);
  const pointRef = react.exports.useRef({
    x: 0,
    y: 0
  });
  const virtualRef = react.exports.useRef({
  ...
  const longPressTimerRef = react.exports.useRef(0);
  const clearLongPress = react.exports.useCallback(
  ...
  return /* @__PURE__ */ react.exports.createElement(react.exports.Fragment, null, /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$export$b688253958b8dfe7, _extends({}, menuScope, {
    virtualRef
  })), /* @__PURE__ */ react.exports.createElement($8927f6f2acc4f386$export$250ffa63cdc0d034.span, _extends({
...
const $1b0217ee4a91d156$export$14afd0be83e45060 = (props) => {
  ...
  const menuScope = $1b0217ee4a91d156$var$useMenuScope(__scopeContextMenu);
  return /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$export$602eac185826482c, _extends({}, menuScope, portalProps));
};
const $1b0217ee4a91d156$var$CONTENT_NAME = "ContextMenuContent";
const $1b0217ee4a91d156$export$572205900c9369e = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopeContextMenu, ...contentProps } = props;
  const context = $1b0217ee4a91d156$var$useContextMenuContext($1b0217ee4a91d156$var$CONTENT_NAME, __scopeContextMenu);
  const menuScope = $1b0217ee4a91d156$var$useMenuScope(__scopeContextMenu);
  const hasInteractedOutsideRef = react.exports.useRef(false);
  return /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$export$7c6e2c02157bb7d2, _extends({}, menuScope, contentProps, {
    ref: forwardedRef,
    side: "right",
    sideOffset: 2,
    align: "start",
...
const $1b0217ee4a91d156$export$16a26dc176a49100 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopeContextMenu, ...itemProps } = props;
  const menuScope = $1b0217ee4a91d156$var$useMenuScope(__scopeContextMenu);
  return /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$export$6d08773d2e66f8f2, _extends({}, menuScope, itemProps, {
    ref: forwardedRef
  }));
});
const $1b0217ee4a91d156$export$8d6b009fadfe1207 = /* @__PURE__ */ react.exports.forwardRef((props, forwardedRef) => {
  const { __scopeContextMenu, ...separatorProps } = props;
  const menuScope = $1b0217ee4a91d156$var$useMenuScope(__scopeContextMenu);
  return /* @__PURE__ */ react.exports.createElement($6cc32821e9371a1c$export$1ff3c3f08ae963c0, _extends({}, menuScope, separatorProps, {
    ref: forwardedRef
  }));
});
...
function Root({
  ...
  return /* @__PURE__ */ jsxs($1b0217ee4a91d156$export$be92b6f5f03c0fe9, {
    children: [/* @__PURE__ */ jsx($1b0217ee4a91d156$export$41fb9f06171c75f4, {
      children: children2
    }), /* @__PURE__ */ jsx($1b0217ee4a91d156$export$602eac185826482c, {
      children: /* @__PURE__ */ jsx($1b0217ee4a91d156$export$7c6e2c02157bb7d2, {
...
function Item({
  ...
  return /* @__PURE__ */ jsx($1b0217ee4a91d156$export$6d08773d2e66f8f2, {
...
function Divider({
  ...
  return /* @__PURE__ */ jsx($1b0217ee4a91d156$export$1ff3c3f08ae963c0, {
    className: className + " mx-4 my-1 h-[1px] bg-gradient-to-r from-indigo-500 to-blue-400",
    ...props
  });
}
function Playground() {
  const editor = react.exports.useRef(null);
  const useStore2 = useGraph({
    ref: editor,
    nodes: [],
    edges: []
  ...
  const nodes = useStore2((state) => state.nodes);
  const edges = useStore2((state) => state.edges);
  const insertNode = useStore2((state) => state.insertNode);
  const updateNodes = useStore2((state) => state.updateNodes);
  const updateEdges = useStore2((state) => state.updateEdges);
  const onConnect = useStore2((state) => state.onConnect);
  const [trackMouse, shouldTrackMouse] = react.exports.useState(true);
  ...
  const createNode = (constructor) => (_) => {
    ...
    const node2 = constructor(String(Date.now()), void 0, opts);
    shouldTrackMouse(true);
    insertNode(node2);
  };
  const menuItems = [{
    ...
    label: "oscillator",
    shortcut: "O",
    onSelect: createNode(asReactFlowNode$3),
    classes: "rounded-t"
  }, {
    type: "item",
    label: "amp",
    shortcut: "A",
    onSelect: createNode(asReactFlowNode$7)
  }, {
    type: "divider"
  }, {
    type: "item",
    label: "xy pad",
    onSelect: createNode(asReactFlowNode$2)
  }, {
    type: "divider"
  }, {
    type: "item",
    label: "const",
    onSelect: createNode(asReactFlowNode$6)
  }, {
    type: "item",
    label: "add",
    shortcut: "+",
    onSelect: createNode(asReactFlowNode$8)
  }, {
    type: "item",
    label: "mul",
    shortcut: "*",
    onSelect: createNode(asReactFlowNode$4)
  }, {
    type: "divider"
  }, {
    type: "item",
    label: "dac",
    shortcut: "D",
    onSelect: createNode(asReactFlowNode$5),
    classes: "rounded-b"
  }];
      ...
      case "item":
        return /* @__PURE__ */ jsxs(Item, {
          ...
          children: [/* @__PURE__ */ jsx("span", {
            ...
            children: label
          }), shortcut && /* @__PURE__ */ jsxs("kbd", {
            className: "text-xs w-10 font-mono text-right text-neutral-500 group-hover:text-white",
            children: ["\u2318+", shortcut]
          })]
        }, label);
      case "divider":
        return /* @__PURE__ */ jsx(Divider, {}, label || i);
      default:
        return null;
    }
  });
  return /* @__PURE__ */ jsx(Root, {
    ...
    children: /* @__PURE__ */ jsx("div", {
      ...
      children: /* @__PURE__ */ jsx(GraphContext.Provider, {
        value: useStore2,
  ...
  children: /* @__PURE__ */ jsx(BrowserRouter, {
    children: /* @__PURE__ */ jsxs(Routes, {
      children: [/* @__PURE__ */ jsx(Route, {
        ...
        path: "/play",
        element: /* @__PURE__ */ jsx(Playground, {})
      }), /* @__PURE__ */ jsx(Route, {
        path: "*",
